...
void elexer_init(const char * buf, int size) {
  lexbuf      = buf;
  lexbuf_size = size;
  lexbuf_pos  = 0;
  lex_mode    = NORMAL;
...
int elexer_get_token(char * tok, int tok_size) {
  int tok_pos = 0;
  int state = 0;
  char c;
...
char *elexer_decode_entities (const char *tok)
{
  char *buf = calloc (1, strlen (tok) + 1);
  char *bp = buf;
  char c;
